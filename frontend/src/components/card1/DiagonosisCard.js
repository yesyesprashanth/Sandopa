...
const DiagonosisCard = ({ rows }) => {
  ...
  // Filter rows based on the "school" field when Search button is clicked
  const handleSearch = () => {
  ...
  // Dynamically generate columns from the keys in the first row, then append the "Actions" column.
    ...
    <Card sx={{ color: "#000", width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
      <CardContent sx={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>
        ...
        <Grid container spacing={1} alignItems="center" sx={{ maxWidth: 500, margin: "auto" }}>
          ...
          <Grid item xs={3}>
            <Button variant="contained" color="primary" fullWidth onClick={() => navigate("/diagonosis")}>
              Create
            </Button>
          </Grid>
        </Grid>
