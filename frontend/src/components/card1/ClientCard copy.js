...
const ClientCard = ({ rows }) => {
  ...
  // Filter rows based on the "name" field when Search button is clicked
  const handleSearch = () => {
  ...
  // then append a custom "Actions" column.
  const columns = useMemo(() => {
    if (!rows || rows.length === 0) return [];
    
    const generatedColumns = Object.keys(rows[0]).map((key) => {
      // Optionally adjust flex for specific fields
